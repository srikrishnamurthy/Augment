...
-(void) setJson:(NSString*) jsonString
{
	...
	else if(json[@"Scan_Error"] != NULL)
	{
		...
		detail = [[UILabel alloc] initWithFrame:CGRectMake(60,margin+90, dWidth - 120, 100)];
	...
	else
	{
		...
		cross.image = [UIImage imageNamed:@"ripe.png"];
		[scroll addSubview:cross];
		
		detail = [[UILabel alloc] initWithFrame:CGRectMake(30,margin+250, dWidth - 60, 100)];
	...
	[NSURLConnection sendAsynchronousRequest:request queue:queue completionHandler:^(NSURLResponse *response, NSData *data, NSError *error)
	 {
		 ...
		 else
		 {
			 dispatch_async(dispatch_get_main_queue(), ^{
				 ...
				 //printf("ASDF\n");
				 __block __strong NSString * x = nil;
				 Firebase *myRootRef = [[Firebase alloc] initWithUrl:@"https://numberadd.firebaseio.com/numberadd"];
				 Firebase * ref2 = [[Firebase alloc] initWithUrl:@"https://foodname.firebaseio.com/"];
				 ...
				 [myRootRef observeEventType:FEventTypeValue withBlock:^(FDataSnapshot *snapshot) {
					 ...
					 if ([snapshot.value isEqualToString: @"1"]){
						 NSLog(@"GU");
						 [ref2 setValue:@"Sprite"];
						 detail.text = @"Sprite; 140 calories";
					 }
					 else if ([snapshot.value isEqualToString: @"2"]){
						 [ref2 setValue:@"Sunkirst"];
						 detail.text = @"Sunkirst; 170 calories";
					 }
					 else if ([snapshot.value isEqualToString: @"3"]){
						 [ref2 setValue:@"Water"];
						 detail.text = @"Water; 0 calories";
					 }
					 else if ([snapshot.value isEqualToString: @"4"]){
						 [ref2 setValue:@"Glasses"];
						 detail.text = @"Glasses; null calories";
					 }
					 else if ([snapshot.value isEqualToString: @"5"]){
						 [ref2 setValue:@"Shoes"];
						 detail.text = @"Shoes; null calories";
					 }
					 else if ([snapshot.value isEqualToString: @"6"]){
						 [ref2 setValue:@"Human Face"];
						 detail.text = @"Human Face; null calories";
					 }
					 else if ([snapshot.value isEqualToString: @"7"]){
						 [ref2 setValue:@"Doritoes"];
						 detail.text = @"Doritoes Chips; 140 calories";
					 }
					 else if ([snapshot.value isEqualToString: @"8"]){
						 [ref2 setValue:@"Popcorn Chips"];
						 detail.text = @"Popcorn Chips; 140 calories";
					 }
					 else if ([snapshot.value isEqualToString: @"9"]){
						 [ref2 setValue:@"Lays Chips"];
						 detail.text = @"Lays Chips; 160 calories ";
					 }
					 else if ([snapshot.value isEqualToString: @"a"]){
						 [ref2 setValue:@"Oreos"];
						 detail.text = @"Oreo Cookies; 270 calories";
					 }
					 else if ([snapshot.value isEqualToString: @"b"]){
						 [ref2 setValue:@"Fritos Chips"];
						 detail.text = @"Fritos Chips; 160 calories";
					 }
					 else if ([snapshot.value isEqualToString: @"c"]){
						 [ref2 setValue:@"Rice Krispie Treats"];
						 detail.text = @"Rice Krispie Treats; 90 calories";
					 }
				 ...
				 /*
				  NSLog(x);
				  NSLog(@"GU");
				  [myRootRef setValue:x];
				  __block __strong NSMutableString* ans = [[NSMutableString alloc] init];
				  int cnt = 0, flag = 0;
				  for (int g=0; g< [x length]; g++){
				  if ([x characterAtIndex:g] == '\"'){
				  cnt ++ ;
				  }
				  if (cnt == 7 && [x characterAtIndex:g] == '\"'){
				  flag = 1; continue;
				  }
				  if (cnt == 8) break;
				  if (flag) {[ans appendFormat:@"%c", [x characterAtIndex:g]]; }
